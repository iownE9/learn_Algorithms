...
public class Percolation {
    ...
    private void unionToAdjoin(int row1, int col1, int row2, int col2) {
        ...
        if (row2 > n || col2 > n || row2 <= 0 || col2 <= 0)
            return;
    ...
    private void fullToAdjoin(int row2, int col2) {
        if (row2 > n || col2 > n || row2 <= 0 || col2 <= 0)
            return;
        ...
        if (isOpen(row2, col2)) {
            isfull[translate(row2, col2)] = true;
            if (row2 == n) {
                isPercolation = true;
            }
            fullToAdjoin(row2 - 1, col2);
            fullToAdjoin(row2 + 1, col2);
            fullToAdjoin(row2, col2 + 1);
            fullToAdjoin(row2, col2 - 1);
        }
    }
